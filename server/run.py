...
DATABASE='camps'#数据库名称
USER='root'#数据库用户
HOST='127.0.0.1'
PASSWORD=''#数据库密码
img_path = 'G:\Desktop\Camps\server\imgs\\'
...
def goods():
    ...
    用POST方法返回商品信息
    """
    db = pymysql.connect(host=HOST,user=USER,password=PASSWORD,db=DATABASE) #用用户名、密码登录数据库
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', debug=True, port=8100,threaded=True)
    app.run(host='192.168.0.101', debug=True, port=8100,threaded=True)
