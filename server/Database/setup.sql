...
`SALES_BY_DAY` AS 
	select
	    date_format(`order`.`otime`, '%Y%m%d') AS `days`,
	    sum(`order`.`totalprice`) AS `sales`
	from `order`
	group by
`DAYS`; 
...
`USER_GOOD` AS 
	select
	    ...
	    `good`.`gname` AS `gname`,
	    round(
	        avg(`order_detail`.`score`),
	        1
	    ) AS `avg_score`,
	    sum(`order_detail`.`quantity`) AS `sales_volume`
	from (`good`
	    join `order_detail`)
	where ( (
	            `good`.`gid` = `order_detail`.`gid`
	        )
	        and (`good`.`gonsale` = 1)
	    )
	group by `good`.`gid`
...
CREATE TRIGGER `CHECKPRICE_INSERT` BEFORE INSERT ON 
`GOOD` FOR EACH ROW BEGIN 
	IF NEW.gprice >=0 THEN SET @s = 'correct price!';
	ELSE SET @s = 'negaive price!';
...
CREATE TRIGGER `CHECKPRICE_UPDATE` BEFORE UPDATE ON 
`GOOD` FOR EACH ROW BEGIN 
	IF NEW.gprice >=0 THEN SET @s = 'correct price!';
	ELSE SET @s = 'negaive price!';
	SIGNAL SQLSTATE '45001' SET MESSAGE_TEXT = @s;
...
CREATE TRIGGER `CHECKPHONE` BEFORE UPDATE ON `USER` 
FOR EACH ROW BEGIN 
	IF ISNULL(NEW.uphone)
	and ISNULL(OLD.uphone) THEN
	SET @s = 'null phone!';
	ELSE IF NEW.uphone=OLD.uphone THEN SET @s = 'right phone!';
	ELSE IF NEW.uphone REGEXP '[1][35678][0-9]{9}' THEN
	SET @s = 'right phone!';
	ELSE SET @s = 'wrong phone!';
	SIGNAL SQLSTATE '45001' SET MESSAGE_TEXT = @s;
	END IF;
	END IF;
	END IF;
